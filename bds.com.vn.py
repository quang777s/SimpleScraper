...
def process_html(response):
    ...
    if not titles:
        if not response.h2: return {'retry': False}
        if not response.h2.a: return {'retry': False}
        url = 'https://batdongsan.com.vn' + response.h2.a['href']
        return {'retry': url}

    for field in ['road', 'direction', 'front', 'floor', 'bedroom', 'restroom', 'furniture', 'user_name', 'user_address', 'user_phone', 'user_mobile', 'user_email']:
        info[field] = ''
    ...
    info['date'] = more_info.find_all('div')[3].getText().replace('Ngày đăng:','').strip('\r\n')
...
def write_to_file(info):
    with open('result.sql', mode='a') as outfile:        
        outfile.write('INSERT INTO info VALUES(NULL, "' + info['title'] + '", "' + info['desc'] + '", "' + info['ad_type'] + '", "' + info['address'] + '", "' + 
                        info['price'] + '", "' + info['area'] + '", "' + info['imgs'] + '", "' + info['road'] + '", "' + info['direction'] + '", "' + 
                        info['front'] + '", "' + info['floor'] + '", ' + info['bedroom'] + ', ' + info['restroom'] + ', "' + info['furniture'] + '", "' + 
                        info['user_name'] + '", "' + info['user_address'] + '", "' + info['user_phone'] + '", "' + info['user_mobile'] + '", "' + info['user_email'] + '", "' + info['date'] + '", NULL);\n')
...
def scrape():
    ...
    while current_id < END_ID:
        ...
        print(detail)
        if detail['retry']: detail = get_detail(CURL_TEMPLATE_RETRY.replace('{url}', detail['retry']))
        if 'info' in detail: write_to_file(detail['info'])
        current_id += 1
